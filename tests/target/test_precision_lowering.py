...
def test_precision_lowering_option_defaults_are_target_specific() -> None:
    assert not SpecializationOptions().enable_precision_lowering
    assert magia_workflow_options().enable_precision_lowering
    assert not magia_workflow_options(
        enable_precision_lowering=False
    ).enable_precision_lowering
...
def test_precision_lowering_requires_cast_and_target_assignments(
    ...
    with pytest.raises(ValueError) as error:
        plan_magia_model(_gemm_model(), mesh, _quiet_magia_options())
...
def test_precision_lowering_serializes_deterministic_artifacts_and_provenance(
    ...
    first = plan_magia_model(
        _gemm_model(),
        magia_mesh(width=3, height=1),
        _quiet_magia_options(),
    )
    second = plan_magia_model(
        _gemm_model(),
        magia_mesh(width=3, height=1),
        _quiet_magia_options(),
    )
