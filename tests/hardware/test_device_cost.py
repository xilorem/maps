...
from maps.target.magia import REDMULE_DEVICE as MAGIA_REDMULE_DEVICE
...
from maps.target.magia.devices import REDMULE_ARRAY_HEIGHT, REDMULE_ARRAY_WIDTH
...
def test_tile_can_use_a_test_local_scalar_device() -> None:
    tile = Tile(
        tile_id=0,
        x=0,
        y=0,
...
def test_gemm_cost_uses_systolic_device_when_available() -> None:
    ...
    assert model.cost(tile_work, redmule_tile, MAGIA_REDMULE_DEVICE) < model.cost(
        tile_work, scalar_tile, TEST_SCALAR_DEVICE
    )
