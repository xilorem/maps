...
def test_pack_weights_orders_by_pipeline_id_converts_floats_and_aligns() -> None:
    ...
    packed = pack_weights(pipeline, constants)
...
def test_pack_weights_rejects_constant_without_pipeline_tensor_id() -> None:
    pipeline = Pipeline("model", _mesh())
    constants = ConstantStore((
        Constant("missing", TensorDType.UINT8, (1,), b"\x01"),
    ))

    with pytest.raises(ValueError, match="no pipeline tensor ID"):
        pack_weights(pipeline, constants)
